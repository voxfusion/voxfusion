...
#[tauri::command]
pub fn process_audio_file(path: String, quality: String) -> Result<Vec<u8>, String> {
    // "high" = raw bytes, no processing
    if quality == "high" {
        return std::fs::read(&path).map_err(|e| format!("Failed to read audio file: {}", e));
    }

    let target_sample_rate: u32 = match quality.as_str() {
        "medium" => 16000,
        "low" => 8000,
        _ => return std::fs::read(&path).map_err(|e| format!("Failed to read audio file: {}", e)),
    };
    ...
    // Mix to mono if stereo (or more channels): average all channels per frame
    ...
    // Resample from source_rate to target_sample_rate using linear interpolation
    ...
    // Write the resampled audio to a new WAV in memory
